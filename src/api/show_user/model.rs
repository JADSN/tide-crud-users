...
use super::{
    outcome::{InternalMessage, UserId},
    ShowUser,
};
...
impl Model<DatabaseConnection, InternalMessage, MvpError> for ShowUser {
    fn model(
        ...
        let user_id: UserId = match serde_json::from_str(&request_body) {
            Ok(id) => id,
            Err(_) => {
        ...
        let retrieved_user = InternalMessage::retrieve_user(db_connection, user_id.id())?;
